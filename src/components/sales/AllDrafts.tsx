...
export default function AllDrafts({ handleEditDraft }: any) {
    ...
    const handleMenuClick = (
        ...
        setSelectedDraftId(Number(item.id))
    ...
    const fetchOrderData = async () => {
        ...
        try {
            ...
            const orderResponse = await posOrders.getAll()
            ...
            let drafts = orderResponse?.data?.filter((order: any) => order.status === ORDER_STATUS.DRAFT)
            if (drafts?.length === 0) {
                showAlert({
                    title: "Error",
                    text: "No order in draft found",
                    severity: "error"
                })
                return;
            }
            setFetchedOrders(drafts);
            setOrders(drafts);
        } catch (error: any) {
    ...
    const handleDeleteOrder = async () => {
        ...
        try {
            const res = await posOrders?.getOne(selectedDraftId)
            if (res?.status === "error") {
                showAlert({
                    title: "Error",
                    text: res?.message ||
            ...
            if (res?.data?.orderItems?.length > 0) {
                ...
                const response = await posOrderItems.delete(orderDetails.orderItems)
                console.log("delete order items response", response)
            }
            const orderResponse = await posOrders.delete(selectedDraftId)
            console.log("orderResponse", orderResponse)
            if (orderResponse.status === "error") {
                showAlert({
                    title: "Error",
    ...
    return (
        <>
            ...
            <ConfirmationModal
                ...
                message={`Are you sure you want to delete this order from draft?`}
                title={"Delete Order from draft"}
            />
            <Box
                display={"flex"}
