  ...
  try {
    ...
    for (const item of data) {
      ...
      if (!name || !costPrice || !sellingPrice || !expiryDate) {
        return { success: false, message: "All fields are required for each product" };
      }

      if (sellingPrice < costPrice) {
        return { success: false, message: `Selling price must be greater than or equal to cost price for product ${name}` };
      }
    ...
    return { success: true, message: `${productsToInsert.length} product(s) added successfully` };
  } catch (err: any) {
    console.error(err);
...
export const getProuctById = async (id: string) => {
  try {
    ...
    return { success: true, data: JSON.parse(JSON.stringify(productResult)) };
  } catch (err: any) {
    console.log(err);
    return { success: false, message: err?.message || "An error occurred" };
  }
